...
public static class Unifier
{
            ...
            switch (leftType, rightType)
            {
                ...
                case (TypeVar tv, GsType resolved):
                    if (OccursIn(tv.Id, resolved))
                        throw new Exception(
                            $"{Position(constraint)}type error: infinite type — '{tv.Id}' occurs in '{resolved}'");
                ...
                default:
                    throw new Exception(
                        $"{Position(constraint)}type mismatch: expected '{leftType}', got '{rightType}'");
            }
        }
    ...
    {
        return constraint.Line > 0 ? $"{constraint.Line}: " : "";
    }
    ...
    /// </summary>
    private static bool OccursIn(string typeVarId, GsType type)
    {
        return type switch
        {
