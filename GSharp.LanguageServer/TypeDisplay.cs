...
public static class TypeDisplay
{
    ...
    private static string Render(GsType type, Dictionary<string, string> generics)
    {
        return type switch
        {
    ...
    {
        return parameter is FunctionType ? $"({Render(parameter, generics)})" : Render(parameter, generics);
    }
